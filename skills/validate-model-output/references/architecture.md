...
| 파랑·verify 다축 | mesh+점관측 SAMPLE · 원형통계 · timeseries · diff_map · 해역별 분석 | 완료 |

결정적 코어: 같은 입력 → 같은 `report`(재현 가능). 미공개 원본 데이터에 의존하지 않는 설계.
(테스트·인수 하니스와 합성 fixture는 이 배포 번들에서 제외됨.)
...
- `references/research/` — 검증·분석 방법 카탈로그 (~500 방법카드, ~115 그림카드; $SKILL 기준 내부 경로)
